...
const Ink = {
  ...
  startRecorder(ctx, bits) {
    //bits = bits per second for video quality, defaults to 2.5Mbps
    let canvas = ctx.canvas;
    this.recorder = new CanvasRecorder(canvas, bits);
    this.recordState = "recording";
    console.log(
      `Beginning to record, stop by calling "Ink.save([name])" or "Ink.stop()"`
    );
  },
  ...
  capture(options) {
    ...
    if (frameRate in window && Math.round(frameRate()) !== 1) frameRate(1);
  ...
  applyImage(ctx) {
    ...
    if (ctx.canvas.width < ctx.canvas.height) {
      let angle =
        ((this.options.orientation === "left" ? 1 : -1) * Math.PI) / 2;
      c.rotate(angle);
    }
